-- Remove a restricao de tipo_transacao IN ('P','V','S')/NULL do calculo de faturamento
-- (documentos considerados vendas). A partir de agora TODO tipo_transacao conta, restando
-- apenas cod_natureza_operacao = '10030' como filtro de natureza da operacao. Ver CLAUDE.md,
-- secao "Filtro padrao Microvix (vendas)".
--
-- Isso muda a materialized view mv_microvix_vendas (fonte de vw_customer_ranking, ja
-- recriada pela migration anterior, descontinuar_store_serie_rules.sql). As demais queries
-- do filtro (people.py, routes/auth.py, routes/mobile.py) sao alteradas no codigo Python,
-- sem precisar de migration.
--
-- Rodar manualmente: psql $PG_DSN -f migrations/remover_filtro_tipo_transacao_mv_vendas.sql
--
