-- Correcao pedida pelo usuario (2026-09): faturamento na loja pode ter tanto pessoa fisica
-- quanto pessoa juridica (ex: funcionario de empresa comprando no balcao, nota emitida pro
-- CNPJ da empresa) — nao e correto tratar "venda na loja" como sinonimo de "cliente PF".
--
-- Remove a restricao (cf.tipo_cliente IS NULL OR cf.tipo_cliente = 'F') de mv_microvix_vendas
-- (fonte de vw_customer_ranking) — o LEFT JOIN a microvix_clientes_fornecedores so existia
-- para essa restricao e fica sem uso, entao tambem e removido. Isso passa a contar compras
-- faturadas para PJ no total_spent/visits_with_purchase de uma pessoa reconhecida pela camera
-- (o recorte PF-only nunca fazia sentido aqui: quem faz a compra fisicamente na loja e a
-- pessoa reconhecida, independente de a nota ter sido emitida no CPF ou no CNPJ dela).
--
-- As demais queries que tinham o mesmo recorte PF-only (people.py: kpi_microvix,
-- faixa_horaria, ticket_por_tipo, top5_por_tipo, ticket_medio_pessoas, compras_recentes_pessoa,
-- compras_recentes_pessoa_detalhe; routes/auth.py e routes/mobile.py: _compute_charts_data)
-- ja foram ajustadas no codigo Python, sem precisar de migration. Os relatorios deliberadamente
-- PJ (top10_clientes_vendedor, top10_produtos_vendedor, top10_clientes_loja,
-- concentracao_clientes_mensal, e o split loja/pedidos de faturamento_mensal) continuam
-- inalterados — sao analises especificas de clientes PJ, nao o faturamento geral da loja.
--
-- Rodar manualmente: psql $PG_DSN -f migrations/faturamento_loja_inclui_pf_e_pj.sql
--
...
-- Recria vw_customer_ranking, identica a definicao anterior (so foi derrubada pelo CASCADE acima).
