-- Redefine o que conta como venda valida em mv_microvix_vendas (fonte de vw_customer_ranking):
-- troca a exclusao de tipo_transacao='J' por dois criterios novos, pedidos pelo usuario (2026-09):
--   1) transacao_pedido_venda = 0 OR IS NULL — exclui transacoes originadas de pedido de venda
--      (transacao_pedido_venda aponta pro numero da transacao do pedido; 0/NULL = venda direta,
--      nao originada de pedido)
--   2) forma_pix = true OR forma_cartao = true OR forma_dinheiro = true — restringe as formas de
--      pagamento aceitas em loja a Pix, Cartao e Dinheiro (exclui Cheque, Cheque a prazo,
--      Crediario, Convenio, Deposito bancario)
-- As demais queries do filtro (people.py, routes/auth.py, routes/mobile.py) ja foram alteradas
-- no codigo Python, sem precisar de migration.
--
-- Rodar manualmente: psql $PG_DSN -f migrations/filtro_forma_pagamento_e_transacao_pedido.sql
--
