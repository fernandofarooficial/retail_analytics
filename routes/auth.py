...
def clientes():
    ...
    if store_scope_ids:
        ...
        for r in rows:
            ...
            compras = _compras_recentes_pessoa_detalhe(r['person_id'])
    ...
    if not selected_store_id and 'company_id' not in request.args:
        ...
        if saved and saved['last_store_id']:
            ...
            if user_type == 'adm':
                ...
                if row:
                    return redirect(url_for('auth.ranking', company_id=row['company_id'], store_id=last_sid))
            elif user_type == 'man':
                row = db.query_one("""
                ...
                """, (last_sid, user_id))
                if row:
                    return redirect(url_for('auth.ranking', store_id=last_sid))
            elif user_type == 'emp':
                row = db.query_one(
    ...
    if user_type == 'adm':
        ...
        if selected_company_id:
            ...
            if match:
                ...
                company_name = match['company_name']
            stores = db.query_all(
                "SELECT store_id, store_name FROM faciais.stores WHERE company_id = %s ORDER BY store_name",
                (selected_company_id,))
    elif user_type == 'man':
        companies = db.query_all("""
            SELECT DISTINCT c.company_id, c.company_name, ct.logo_url
...
def ranking_pessoa(person_id):
    ...
    store = _resolve_store_for_user(user_id, user_type, store_id)
    if not store:
        return redirect(url_for('auth.ranking'))
