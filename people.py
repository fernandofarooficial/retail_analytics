...
def vendedores_mes(portal, cnpj, mes_ini_cur, mes_fim_cur, mes_ini_ant, mes_fim_ant):
    ...
    rows = db.query_all("""
        ...
        WHERE  m.portal    = %s
          ...
          AND  (m.transacao_pedido_venda = 0 OR m.transacao_pedido_venda IS NULL) AND (m.forma_pix = true OR m.forma_cartao = true OR m.forma_dinheiro = true)
...
def produtos_por_pessoa(store_id, person_id, cnpj, days):
    ...
    Junta por (cnpj_emp, serie, documento) quando person_purchases.serie está
    preenchida (2026-09); cai pra (cnpj_emp, documento) nas linhas antigas sem esse
    campo — mesmo cuidado de ambiguidade documentado no CLAUDE.md."""
    return db.query_all("""
...
def ticket_medio_pessoas(person_ids):
    ...
    pelo menos uma nota. Junta por (cnpj_emp, serie, documento) quando
    person_purchases.serie está preenchida (2026-09 — coluna nova do camera300,
    backfill parcial); cai pra (cnpj_emp, documento) nas linhas antigas sem serie —
    mesmo cuidado documentado no CLAUDE.md."""
    if not person_ids:
        return {}
    rows = db.query_all("""
        WITH compras AS (
...
def compras_recentes_pessoa(person_id, max_dias=5):
    ...
    consulta a microvix_movimento, casada por cnpj_emp+serie+documento quando
    person_purchases.serie está preenchida — 2026-09; cai pra cnpj_emp+documento
    nas linhas antigas sem esse campo). Ver CLAUDE.md — documento sozinho não
    identifica a NF."""
    rows = db.query_all("""
        WITH compras AS (
            SELECT
...
def _resolver_link(link, store_cnpj):
    ...
    Grava serie da própria NF confirmada (2026-09, coluna nova do camera300) —
    o vínculo manual já sabe a série exata digitada pelo usuário, então não há
    ambiguidade aqui."""
    nota = db.query_one("""
        SELECT 1
        FROM   microvix.microvix_movimento
    ...
    if existing is None:
        db.execute("""
            INSERT INTO faciais.person_purchases (person_id, store_id, bill, serie, is_identified)
            VALUES (%s, %s, %s, %s, TRUE)
        """, (link['person_id'], link['store_id'], link['numero_nota'], link['serie']))
    elif existing['person_id'] != link['person_id']:
        db.execute("""
            UPDATE faciais.person_purchases
            SET    person_id = %s, serie = %s, is_identified = TRUE, is_cancelled = FALSE
            WHERE  person_purchase_id = %s
        """, (link['person_id'], link['serie'], existing['person_purchase_id']))
    else:
        db.execute("""
            UPDATE faciais.person_purchases
            SET    serie = %s
            WHERE  person_purchase_id = %s AND serie IS NULL
        """, (link['serie'], existing['person_purchase_id']))
